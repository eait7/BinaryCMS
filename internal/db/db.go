...
func safeAlterColumns(db *sql.DB) {
	alters := []string{
		...
		`ALTER TABLE pages ADD COLUMN hide_title BOOLEAN DEFAULT 0`,
		`ALTER TABLE pages ADD COLUMN hide_breadcrumbs BOOLEAN DEFAULT 0`,
...
func seedDefaults(db *sql.DB) {
	defaults := map[string]string{
		...
		"site_favicon_icon": "brand-abstract",
